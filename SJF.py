...
#now we have to select 1st process and in SJF 1st process is on basis of FCFS and burst time
#let 1st process is at starting index
first_procoess_arrival = processes[0].arrival_time
first_process_index = 0

#now comparing 1st process with remaining processes on the basis of arrival time and burst time
for i in range(1,int(count)):
	...
	#if arrival time is equal, compare on the basis of burst time
	elif processes[i].arrival_time==first_procoess_arrival:
                if processes[i].burst_time < processes[first_process_index].burst_time:
                    first_process_arrival = processes[i].arrival_time
                    first_process_index = i
...
#sorting remaining processes on bases of shortest burst time
